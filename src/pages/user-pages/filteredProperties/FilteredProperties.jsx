...
export const FilteredProperties = () => {
  ...
  const filteredProps = newProps.filter((item) => {
    const rooms = item?.roomTypeResourceList
    var bedsAvailable = 0
    for(var x=0;x<rooms.length;x++){
      bedsAvailable+=rooms[x].bedsAvailable
    }
    item["img"] = item["imageSrc"]
    item["transportationFacility"] = "Free airport taxi"
    item["cancellationOptions"] = "Free Cancellation" 
    const hotelAvailableStartDate = new Date(item.availableFrom);
    const hotelAvailableEndDate = new Date(item.availableTo);
    const userRequiredStartDate = date[0].startDate;
    const userRequiredEndDate = date[0].endDate;
    return (
      item.city === destination &&
      bedsAvailable > 0 &&
          hotelAvailableStartDate <= userRequiredStartDate &&
          userRequiredEndDate <= hotelAvailableEndDate
    );
  });
